...
def get_success_rate(db: Session, workspace_id: str) -> List[SuccessRateData]:
    # 采用与 Overview 一致的“仅统计每个任务最新结果”逻辑
    latest_stmt = (
        db.query(
            SddDashboardMetric.task_id,
            sqlfunc.max(SddDashboardMetric.recorded_at).label("latest_at")
    ...
    results = (
        db.query(SddDashboardMetric.metric_value, sqlfunc.count(SddDashboardMetric.id))
        .join(latest_stmt, 
            (SddDashboardMetric.task_id == latest_stmt.c.task_id) & 
            (SddDashboardMetric.recorded_at == latest_stmt.c.latest_at)
        )
        .filter(SddDashboardMetric.metric_type == "TASK_RESULT")
        .group_by(SddDashboardMetric.metric_value)
        .all()
    )
    
    # 转换为字典方便处理
    stats = {int(r[0]): r[1] for r in results}
    
    return [
        SuccessRateData(status="DONE", count=stats.get(1, 0)),
        SuccessRateData(status="FAILED", count=stats.get(0, 0)),
    ]
