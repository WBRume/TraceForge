...
def _make_engine() -> TaskAgentEngine:
    engine = TaskAgentEngine.__new__(TaskAgentEngine)
    ...
    engine.is_current = mock.Mock(return_value=True)
    engine.segments = mock.Mock()
    engine.frontend = SimpleNamespace(
        push=mock.AsyncMock(),
        push_status=mock.AsyncMock(),
    )
    return engine


class TaskAgentEngineModelEventTest(unittest.IsolatedAsyncioTestCase):
    async def test_dsh_model_event_updates_snapshot_and_runtime_status(self):
        ...
        model_status = engine.frontend.push_status.await_args_list[-1]
        self.assertEqual(
            model_status.args[:2],
...
class RemoteStopAfterErrorTest(unittest.IsolatedAsyncioTestCase):
    ...
    def _make_engine(self, *, execution_kind="REMOTE_SESSION", session_id="sid-1") -> TaskAgentEngine:
        engine = TaskAgentEngine.__new__(TaskAgentEngine)
        engine.task_id = "task-1"
        engine.ws_id = "workspace-1"
        engine.user_id = "user-1"
