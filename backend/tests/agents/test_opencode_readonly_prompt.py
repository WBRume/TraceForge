...
class _FakeClient:
    ...
    async def post(self, url, **kwargs):
        self._captured.setdefault("calls", []).append((url, kwargs))
...
def test_default_mode_has_no_constraint_prefix():
    ...
    body = captured["kwargs"]["json"]
    assert "agent" not in body
    assert body["text"] == "把会话过程总结为定位结果"


def test_explicit_plan_still_switches_plan_agent():
    captured = _run_send_prompt("plan")
    body = captured["kwargs"]["json"]
    assert captured["calls"][0] == ("http://test/api/session/sess-1/agent", {"json": {"agent": "plan"}})
